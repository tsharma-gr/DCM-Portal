...
export function DashboardCharts({ data }: ChartsProps) {
  ...
  return (
    <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-7 mt-4 print:flex print:flex-col print:gap-8 print:w-full">
      <motion.div
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.4 }}
        className="col-span-full lg:col-span-4 print:w-full print:break-inside-avoid"
      >
        <Card className="h-full bg-card/60 backdrop-blur-md border-border/50 shadow-sm print:border-none print:shadow-none relative overflow-hidden group hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] transition-all duration-300">
          ...
          <CardHeader className="pb-3 pt-5 sm:pb-4 sm:pt-6">
            <CardTitle className="font-heading text-[15px] sm:text-[17px] font-extrabold tracking-tight text-[var(--ink)]">Daily Processing Trend</CardTitle>
          </CardHeader>
            ...
            <div className="h-[220px] sm:h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendData} margin={{ top: 5, right: 10, bottom: 5, left: -20 }}>
                  ...
                  <XAxis dataKey="date" stroke="#888888" fontSize={10} tickLine={false} axisLine={false} tickMargin={8} />
      ...
      <motion.div
        ...
        className="col-span-full lg:col-span-3 print:w-full print:break-inside-avoid"
      >
        <Card className="h-full bg-card/60 backdrop-blur-md border-border/50 shadow-sm print:border-none print:shadow-none relative overflow-hidden group hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] transition-all duration-300">
          <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-pink-500/80 to-rose-500/80"></div>
          <CardHeader className="pb-3 pt-5 sm:pb-4 sm:pt-6">
            <CardTitle className="font-heading text-[15px] sm:text-[17px] font-extrabold tracking-tight text-[var(--ink)]">Classification Overview</CardTitle>
          </CardHeader>
          <CardContent className="px-2 sm:px-6">
            <div className="h-[220px] sm:h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
      ...
      >
        <Card className="h-full bg-card/60 backdrop-blur-md border-border/50 shadow-sm print:border-none print:shadow-none relative overflow-hidden group hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] transition-all duration-300">
          ...
          <CardHeader className="pb-3 pt-5 sm:pb-4 sm:pt-6">
            <CardTitle className="font-heading text-[15px] sm:text-[17px] font-extrabold tracking-tight text-[var(--ink)]">Platform Distribution</CardTitle>
          </CardHeader>
          <CardContent className="px-2 sm:px-6">
            <div className="h-[200px] sm:h-[250px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={platformData} margin={{ top: 5, right: 10, bottom: 5, left: -20 }}>
      ...
      >
        <Card className="h-full bg-card/60 backdrop-blur-md border-border/50 shadow-sm print:border-none print:shadow-none relative overflow-hidden group hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] transition-all duration-300">
          ...
          <CardHeader className="pb-3 pt-5 sm:pb-4 sm:pt-6">
            <CardTitle className="font-heading text-[15px] sm:text-[17px] font-extrabold tracking-tight text-[var(--ink)]">DCM Distribution</CardTitle>
          </CardHeader>
          <CardContent className="px-3 sm:px-6">
            <div className="h-[200px] sm:h-[250px] w-full pr-1">
                <div className="flex flex-col gap-3 overflow-y-auto pr-2 h-[200px] sm:h-[250px] custom-scrollbar">
                  {dcmData.map((dcm, index) => (
                    ...
                    >
                      <div className="flex justify-between items-center text-[12px] font-semibold">
                        <span className="text-[var(--ink)] group-hover:text-[var(--violet)] transition-colors truncate max-w-[70%]">{dcm.name}</span>
                        <span className="text-slate-500 font-mono text-[11px]">{dcm.size.toLocaleString()}</span>
                      </div>
